...
function ConfirmCard({
  ...
  return (
    ...
    >
      <div className={`mb-3 flex h-10 w-10 items-center justify-center rounded-2xl border shadow-inner shadow-white/5 ${iconClass}`}>
      ...
      <h3 id={titleId} className="text-[15px] font-semibold tracking-[-0.01em] text-slate-50">{title}</h3>
      <p id={descriptionId} className="mt-2 whitespace-pre-line text-[13px] leading-5 text-slate-400">{description}</p>
      ...
      <div className="mt-5 grid grid-cols-2 gap-2.5">
        <button
          ...
          onClick={onClose}
          disabled={isLoading}
          className="inline-flex h-10 items-center justify-center rounded-xl border border-white/10 bg-white/5 px-3 text-xs font-semibold text-slate-200 shadow-sm shadow-black/10 transition-all duration-200 hover:border-white/20 hover:bg-white/10 active:scale-95 disabled:cursor-not-allowed disabled:opacity-60"
        >
          {cancelLabel}
        </button>
        <button
          ...
          onClick={onConfirm}
          disabled={isLoading}
          className={`inline-flex h-10 items-center justify-center rounded-xl border px-3 text-xs font-semibold shadow-sm shadow-black/10 transition-all duration-200 active:scale-95 disabled:cursor-not-allowed disabled:opacity-60 ${toneClass}`}
        >
          {isLoading ? (
            <>
  ...
  return createPortal(
    <>
      ...
      <ConfirmCard
        ...
        className="fixed z-[70] max-h-[calc(100dvh-2rem)] overflow-y-auto rounded-[24px] border border-white/12 bg-slate-950/98 p-4.5 shadow-2xl shadow-black/45 ring-1 ring-white/5 animate-in fade-in zoom-in-95 slide-in-from-top-1 duration-150 ease-out transition-[top,left,width] will-change-transform"
        style={{
          top: position.top,
          left: position.left,
